...
Précédant de plusieurs années la directive INSPIRE, les standards dédiés aux données géographiques furent édictés très tôt par l'OGC et répondaient aux besoins de visualisation et de téléchargement adaptés aux différents formats :
...
```{Note}
**API RESTful** : Interface de Programmation d’Applications (API) utilisant le protocole HTTP pour communiquer et permettant de créer des services web interopérables pour consulter, modifier, créer ou supprimer des ressources.
```
